...
import styles from '../../baseline-sheet/[id]/entry.module.css';
...
function migrateSheet(data: any): DailyRoutineSheet {
    const oldRows: any[] = data.rows || [];
    const rows: StepRow[] = oldRows.map((r: any) => ({ step: r.step || r.name || '' }));
        ...
        const sessions: Session[] = data.sessions.map((sess: any) => {
            ...
            rows.forEach((_, i) => {
                ...
                if (r === 'pass' || r === 'fail') {
                    ...
                    // Quick migration logic for old fractional data: >50% is pass
                    const [corr, tot] = r.split('/');
                    const pc = parseInt(corr, 10) / parseInt(tot, 10);
                    if (!isNaN(pc) && pc >= 0.5) results[String(i)] = 'pass';
                    else if (!isNaN(pc)) results[String(i)] = 'fail';
                    else results[String(i)] = null;
                } else {
                    results[String(i)] = null;
                }
            });
            return { ...sess, results };
        });
...
export default function DailyRoutinesEntryPage() {
    ...
    const handleProgramChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        const existing: any[] = await dbClient.get(`/daily_routines`).then(r => r.json()).catch(() => []);
    ...
    const saveEdit = async () => {
        ...
        try {
            ...
            await dbClient.patch(`/daily_routines/${sheet.id}`, updatedSheet);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const updatedSheet: DailyRoutineSheet = { ...sheet, sessions: [...(sheet.sessions || []), sessionToAdd], ...(sheet.id ? {} : { createdAt: new Date().toISOString() }) };
            if (sheet.id) {
                await dbClient.patch(`/daily_routines/${sheet.id}`, updatedSheet);
            } else {
                await dbClient.post('/daily_routines', updatedSheet);
            }
            router.push('/forms/daily-routines');
        } catch {
            alert('Could not save — is the shared database running?');
        } finally {
            setSaving(false);
...
<title>Daily Routines &ndash; ${sheet.clientName} &ndash; ${sheet.program}</title>
<style>
...
<div class="page">
  ...
  <div class="doc-header">
    ...
    <div class="doc-label">Daily Routines Tracker</div>
  </div>
        ...
        <div className={styles.page}>
            ...
            <div className={styles.pageHeader}>
                <div className={styles.headerLeft}>
                    <button className={styles.backBtn} onClick={() => router.push('/forms/daily-routines')}>
            ...
            {sheet && rows.length > 0 && (
                <div className={styles.gridCard}>
                    <div className={styles.gridHeader}>
                        <span className={styles.gridTitle}>Daily Routines Tracker</span>
                        ...
                        <table className={styles.grid}>
                            <thead>
                                <tr>
                                    ...
                                    <th className={styles.stepNameCol}>Step</th>
