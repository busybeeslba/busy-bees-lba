...
export default function BaselineDataTablePage() {
    ...
    useEffect(() => {
        Promise.all([
            ...
            .then(([baselineData, clientData]) => {
                // Assuming baselineData is the primary source for sheets, similar to program_mastery
                // And clientData might be used for something else, or is just fetched alongside.
                // For now, we'll use baselineData to set sheets.
                setSheets(Array.isArray(baselineData) ? baselineData : []);
                // If clientData needs to be stored or processed, add state for it here.
            })
            .catch(error => {
    ...
    // Flatten all sheets → one row per STO per session
    ...
    const getGroupKey = (r: BaselineRow) => {
    ...
    const groups: { key: string; rows: BaselineRow[] }[] = [];
    if (groupBy === 'none') {
    ...
    const groupPassRate = (rows: BaselineRow[]) => {
    ...
    const exportCsv = () => {
        const headers = ['Client', 'Program', 'Day', 'Date', 'Employee', 'STO', 'Result'];
        ...
        const a = document.createElement('a'); a.href = url; a.download = 'baseline-data.csv'; a.click();
    ...
    const COLUMNS: ColumnDef<any>[] = React.useMemo(() => [
        ...
        { id: 'sto', label: 'STO', sortKey: 'sto', renderCell: (r: any) => <td key={`sto-${r.sheetId}`} className={styles.td}><span className={styles.stoName}>{r.stoName}</span></td> },
        { 
            id: 'result', 
            label: 'Result', 
            sortKey: 'result', 
            renderCell: (r: any) => (
                <td key={`result-${r.sheetId}`} className={`${styles.td} ${styles.tdCenter}`}>
                    {r.result === 'pass' ? (
                        <span className={styles.passChip}><span className={styles.resultIconPass}>✓</span> Pass</span>
                    ) : r.result === 'fail' ? (
                        <span className={styles.failChip}><span className={styles.resultIconFail}>✗</span> Fail</span>
                    ) : (
    ...
    const { activeColumns, allColumnsOrdered, hiddenColumnIds, toggleColumnVisibility, moveColumn, resetToDefaults } = useTableSettings('baseline_data_table_config', COLUMNS);
        ...
        <div className={styles.page}>
            ...
            <div className={styles.header}>
                <div className={styles.headerLeft}>
                    <button className={styles.backBtn} onClick={() => router.push('/forms/baseline-sheet')} title="Back to list">
                        <ArrowLeft size={16} />
                    </button>
                    <ClipboardCheck size={22} className={styles.headerIcon} />
                    <div>
                        <h1 className={styles.title}>Baseline Sheet — Data Table</h1>
            ...
            <div className={styles.toolbar}>
                ...
                <div className={styles.toolbarRight}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginLeft: 'auto' }}>
                        ...
                        <button className={styles.filterBtn} onClick={() => setShowSettingsDrawer(true)} title="Page Settings" style={{ padding: '8px', height: '100%' }}>
                                            ...
                                            return (
                                                <tr key={`${r.sheetId}-${r.day}-${r.stoIdx}-${ri}`}
                                                    ...
                                                    <td className={`${styles.td} ${styles.tdCenter}`}>
                                                        <a href={`/forms/baseline-sheet/${r.sheetId}`} className={styles.editLink} onClick={e => e.stopPropagation()}>✏</a>
                                                    </td>
                                                </tr>
                                            );
                                        })}
