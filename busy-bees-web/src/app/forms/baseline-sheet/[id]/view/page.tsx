...
export default function BaselineSheetViewPage() {
    ...
    useEffect(() => {
        ...
        dbClient.get(`/academic_baselines/${params.id}`)
            .then(data => {
                setSheet(data);
                // Initialize all STOs as selected
                if (data?.rows) {
    ...
    const handleDownloadPDF = async () => {
        try {
            ...
            const element = document.getElementById('baseline-sheet-print-container');
            if (!element) return;
            
            const clientName = sheet?.clientName || 'Client';
            const programName = sheet?.program || 'Program';
            const date = new Date().toISOString().split('T')[0];
            const filename = `BaselineSheet_${clientName}_${programName}_${date}.pdf`.replace(/[^a-zA-Z0-9_.-]/g, '_');
    ...
    const filteredSheet = useMemo(() => {
        ...
        const filteredSessions = (sheet.sessions || []).filter((s: any) => {
            if (!s.date) return true; // keep elements without date just in case
    ...
    if (!sheet) {
        return (
            <div style={{ padding: '40px', textAlign: 'center' }}>
                ...
                <button 
                    onClick={() => router.push('/forms/baseline-sheet')}
                    style={{ 
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#f1f5f9', fontFamily: 'system-ui, sans-serif' }}>
            ...
            <div className="no-print" style={{ 
                ...
                <button 
                    onClick={() => router.push(`/forms/baseline-sheet/${params.id}`)}
                    style={{
                    ...
                    <div style={{ display: 'flex', gap: '32px', flexWrap: 'wrap' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', flex: 1 }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                <span style={{ fontSize: '13px', fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Filter Targets / STOs</span>
                            ...
                            <div style={{ 
                                ...
                                {sheet.rows?.map((row: any, i: number) => (
                                    <label key={i} style={{ 
                                        ...
                                        color: selectedItems.has(i) ? '#0f766e' : '#64748b',
                                ...
                                {(!sheet.rows || sheet.rows.length === 0) && <span style={{ color: '#94a3b8', fontSize: '13px' }}>No targets found.</span>}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div style={{ padding: '32px 24px' }}>
                ...
                >
                    <div id="baseline-sheet-print-container">
                        <BaselineSheetPrintView sheet={filteredSheet} printOnly={false} includeGraph={includeGraph} />
                    </div>
                </div>
            </div>
        </div>
