...
export default function MassTrialViewPage() {
    ...
    const handleDownloadPDF = async () => {
        try {
            ...
            const element = document.getElementById('mass-trial-print-container');
            if (!element) return;
            
            const clientName = sheet?.clientName || 'Client';
            const programName = sheet?.program || 'Program';
            const date = new Date().toISOString().split('T')[0];
            const filename = `MassTrial_${clientName}_${programName}_${date}.pdf`.replace(/[^a-zA-Z0-9_.-]/g, '_');
    ...
    if (!sheet) {
        return (
            <div style={{ padding: '40px', textAlign: 'center' }}>
                ...
                <button 
                    onClick={() => router.push('/forms/mass-trial')}
                    style={{ 
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#f1f5f9', fontFamily: 'system-ui, sans-serif' }}>
            ...
            <div className="no-print" style={{ 
                ...
                <button 
                    onClick={() => router.push(`/forms/mass-trial/${params.id}`)}
                    style={{
            ...
            <div style={{ padding: '32px 24px' }}>
                ...
                >
                    <div id="mass-trial-print-container">
                        <MassTrialPrintView sheet={filteredSheet} printOnly={false} includeGraph={includeGraph} />
                    </div>
                </div>
            </div>
        </div>
