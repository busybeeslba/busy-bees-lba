...
export default function ClientsPage() {
    ...
    const COLUMNS: ColumnDef<any>[] = useMemo(() => {
        const baseCols: ColumnDef<any>[] = [
            ...
            { id: 'guardianFullName', label: 'Guardian Full Name', sortKey: 'guardian', renderCell: (client: any) => <td key="guardianFullName" onClick={() => router.push(`/clients/${encodeURIComponent(client.clientId)}`)}><span className={styles.cellText}>{`${client.guardian || ''} ${(client as any).guardianLastName || ''}`.trim()}</span></td> },
